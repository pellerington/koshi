...
bool LightSamplerArea::sample_light(const uint num_samples, const Surface * surface, std::vector<LightSample> &light_samples, Resources &resources)
{
    const Transform3f& obj_to_world = geometry->get_obj_to_world();
...
float LightSamplerArea::evaluate_light(const Intersect * intersect, const Surface * surface, Resources &resources)
{
