...
bool LightSphere::sample_light(const uint num_samples, const Vec3f * pos, const Vec3f * pfar, std::vector<LightSample> &light_samples)
{
    if(elliptoid /* or inside sphere */)
        return sample_area(num_samples, pos, pfar, light_samples);
    return sample_sa(num_samples, pos, pfar, light_samples);
}
...
bool LightSphere::sample_sa(const uint num_samples, const Vec3f * pos, const Vec3f * pfar, std::vector<LightSample> &light_samples)
{
    ...
    for(uint i = 0; i < rnd.size(); i++)
    {
        ...
        const float phi = max_angle * sqrtf(rnd[i][1]);
...
bool LightSphere::sample_area(const uint num_samples, const Vec3f * pos, const Vec3f * pfar, std::vector<LightSample> &light_samples)
{
    ...
    {
        const float theta = TWO_PI * rnd[i][0];
        const float phi = acosf(1.f - 2.f * rnd[i][1]);
